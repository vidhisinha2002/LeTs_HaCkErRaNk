...
int count=0,tall=candles[0];    //initially tallest hm pehle candle ko le lete h
for(int i=0;i<candles_count;i++)
{   
    if (candles[i]>tall)    //agr ele tallest se bara hua to
    {   
        tall=candles[i];    //wo ele he tallest ban jaega
    }
}
for(int i=0;i<candles_count;i++)
{
    if(candles[i]==tall)    //jab ele tallest k barabar mila
    {
        count++;    //inc  count
    }
}
