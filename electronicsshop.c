...
int getMoneySpent(int keyboards_count, int* keyboards, int drives_count, int* drives, int b) {
    int result=-1;      // 0 ni kie kyuki agr fail hua to -1 return krna he
    int maximum=0;
    for(int i=0;i<keyboards_count;i++)
    {
        for(int j=0;j<drives_count;j++)
        {
            ...
            {//uske b se km ya fit hora tb or jb hamare exiting max result is jyada tb
                result=keyboards[i]+drives[j];  
            }
        }
    }
    return result;
    //ab ek array me sare posible store krke max dhond k return krne is better
    //he ki variable k value ko fir dosre value se assign krdo if condition satisfies
