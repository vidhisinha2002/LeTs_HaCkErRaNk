...
int potion=0;
int max=0;//me ye initialize krna bhol gai thi to bht bara glt aara tha
for(int i=0;i<n;i++)
{
    if(max<height[i])//max km nikla to max m wo jyada wala value dal do
        max=height[i];  
}
if(max-k<0) //agr potion -ve aara mtlb 0 potion ni chie
    return 0;
else
{
    potion=max-k;
